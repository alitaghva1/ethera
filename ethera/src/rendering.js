...
function buildEnvironmentLights() {
    ...
    ENV_LIGHTS[0] = [
        ...
        // AI-generated lantern posts flanking road junctions
        { row: 7, col: 13, type: 'torch', color: [255, 190, 90], radius: 45, intensity: 0.55 },
        { row: 7, col: 17, type: 'torch', color: [255, 190, 90], radius: 45, intensity: 0.55 },
        { row: 16, col: 12, type: 'torch', color: [255, 190, 90], radius: 45, intensity: 0.55 },
        { row: 16, col: 18, type: 'torch', color: [255, 190, 90], radius: 45, intensity: 0.55 },
    ];
}
...
function drawDarkness() {
    ...
    if (zoneCfg && zoneCfg.isHell) {
        ...
        const radius = Math.max(5, (lightRadius + flicker) * _abyssLightMult * _bossDim);
    ...
    const radius = Math.max(5, (lightRadius + flicker) * _abyssLightMult2 * _bossDim2);
